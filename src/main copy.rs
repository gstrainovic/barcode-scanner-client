...
#[tokio::main]
async fn get_lager_users(jwt: &str) -> Result<Vec<String>, reqwest::Error> {
...
// Gruppe für Mitarbeiter mit Rolle Lager, um zu fragen welche Kollegen beim Verpacken helfen
fn group_lager(wizard: group::Wizard, glager_users: Vec<String>) -> () {
    // Label Text: "Bitte Kollegen auswählen, die beim Verpacken helfen"
    // Checkboxen mit den Namen der Kollegen
    // Button "Zurück"
    // Button "Weiter"
    ...
    let mut lager_frame = frame::Frame::default()
        .with_label("Bitte Kollegen auswählen, die beim Verpacken helfen");
    grid.insert_ext(&mut lager_frame, 7, 1, 1, 1);

    // checkboxen mit den namen der kollegen aus der array lager_users
    let mut lager_checkboxen = Vec::new();
    for (i, lager_user) in glager_users.iter().enumerate() {
        let mut lager_checkbox = button::CheckButton::default().with_label(&lager_user);
        grid.insert_ext(&mut lager_checkbox, 8 + i as i32, 1, 1, 1);
        lager_checkboxen.push(lager_checkbox);
    }

    //lager_button_zurueck
    let mut lager_button_zurueck = button::Button::default().with_label("Zurück");
    grid.insert_ext(&mut lager_button_zurueck, 9, 1, 1, 1);

    //lager_button_weiter
    let mut lager_button_weiter = button::ReturnButton::default().with_label("Weiter");
    grid.insert_ext(&mut lager_button_weiter, 10, 1, 1, 1);
    ...
    //lager_button_weiter funktion
    lager_button_weiter.set_callback({
        let mut wiz_c = wizard.clone();
        move |_| wiz_c.next()
    });
...
fn main() {
    ...
    let mut glager_users: Vec<String> = Vec::new();
    ...
    group_lager(wizard.clone(), glager_users);
    ...
    login_button.set_callback(move || {
            ...
            Ok(j) => {
                match j {
                    JWT {
                        ...
                        // if rolle is not lager skip lager page
                        if rolle == "Lager" {
                            wizard.next();
                            let lager_users_res = get_lager_users(&gjwt);
                            glager_users = lager_users_res.unwrap();
                            println!("Users: {:?}", glager_users);
                        } else {
                            wizard.next();
                            wizard.next();
                        }
