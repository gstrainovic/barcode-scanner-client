...
fn draw_gallery() {
    let mut tab = Tabs::new(10, 10, 500 - 20, 450 - 20, "");

    let mut grp1 = Group::new(10, 35, 500 - 20, 450 - 45, "Tab1\t\t");

    let mut pack = Pack::new(15, 45, 150, 450 - 45, "");
    pack.set_spacing(10);
    let mut _but1 = Button::default().with_size(0, 30).with_label("Button");
    // go to next tab if press _but1
    ...
    let mut grp2 = Group::new(10, 35, 500 - 30, 450 - 25, "Tab2\t\t");
    grp2.end();
    tab.end();

    // on click _btn1 go to next tab
    _but1.set_callback(move |_| {
        tab.set_value(&grp2);
    });
